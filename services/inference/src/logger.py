...
def save_experiment_to_csv(model_name, mse_mean, t_caut, t_warn, t_err):
    """
    ...
    with open(csv_path, mode='a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['Date', 'Domain', 'Mean_MSE', 'Threshold_Caution', 'Threshold_Warning', 'Threshold_Error'])
        
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        writer.writerow([current_time, model_name, round(mse_mean, 6), round(t_caut, 6), round(t_warn, 6), round(t_err, 6)])