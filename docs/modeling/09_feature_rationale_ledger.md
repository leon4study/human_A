...
이 프로젝트의 모델링 시간이 오래 걸린 핵심 이유는 이렇습니다. 물리적으로 같이 움직여 사실상 같은 정보를 담은 중복 지표(다중공선성 — 서로 강하게 상관된 변수들)를 제거하고 나면, 도메인별로 남는 지표가 빈약해집니다. 그래서 **도메인 지식으로 새 파생 지표를 설계하고 각 지표의 근거를 쌓는 작업**을 반복해야 했습니다. 이 원장은 그 근거를 한 번 정리해 두어 다시 쓰게 합니다.
