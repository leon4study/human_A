...
- **돌발 정지** → 재배사 _전체_ 작물에 동시 수분 미공급 → 회복 불가능한 스트레스
...
### 임계치 — 6시그마 3단계 알람 (분위로 FAR 통제)

복원 오차 MSE 분포에서 **정상의 극단을 3단계로 잡는 6시그마식 알람**을 토대로 합니다. 도메인 공통 기준이라 설비별 공정한 비교가 가능합니다.

| 단계               | 기준(개념) | 의미                 |
| ------------------ | ---------- | -------------------- |
| 🟡 주의 (Caution)  | μ + 2σ     | 운영자 모니터링 강화 |
| 🟠 경고 (Warning)  | μ + 3σ     | 정비 일정 사전 조율  |
| 🔴 치명 (Critical) | μ + 6σ     | 즉시 점검 / 출동     |

다만 도메인 복원오차가 한쪽으로 길게 치우친(right-skew) 분포라, μ+kσ는 도메인마다 대응하는 꼬리 확률이 달라져 **오탐률(FAR)이 들쭉날쭉**해집니다(한 도메인을 맞추면 다른 도메인이 튀는 문제). 그래서 같은 "정상 상위 극단" 철학은 유지하되, 실제 임계는 **분위(percentile P99/99.6/99.9)로 배치해 목표 FAR을 직접 통제**합니다. 피처별 RCA 임계는 2σ/3σ/6σ를 그대로 씁니다.

기동(펌프 가동 직후) 구간은 별도 분위 band로 판정해 정상 기동의 일시적 overshoot는 통과시키고, **"연속 N회 임계 초과"** 디바운싱으로 순간 노이즈 튐을 걸러 헛출동 인건비까지 줄였습니다.

또한 도메인 이상 점수는 피처별 복원오차를 **robust하게 집계**합니다 — 잡음이 큰 한 피처가 점수를 지배해 헛알람을 내지 않도록, 필요한 도메인은 상위 오차를 일부 제외한 평균(trimmed-mean)으로 점수를 냅니다(skew 데이터에서 평균 추정기의 취약성을 보완하는 표준 기법).
