...
type Equipment = {
  ...
  additionalInfo?: Record<string, string>;   // 추가정보 (key-value 형태로 추가 정보를 넣겠다)
};
...
// 물 배관, 양액 배관, 금속 매니폴드/보조 배관
type PipeTone = 'water' | 'nutrient' | 'steel';   

// 배관 색상 테마 정의
const pipePalette: Record<PipeTone, { shell: string; body: string; flow: string; glow: string }> = {
  water: {
    shell: '#020617',   // shell : 가장 바깥 어두운 테두리
    body: '#1d4ed8',   // body : 배관 본체 색
    flow: '#60a5fa',   // flow : 안에서 흐르는 유체 느낌
    glow: '#bfdbfe',   // glow : 약한 광택 / 빛 번짐
  },
  nutrient: {
    shell: '#1c1917',
    body: '#9a3412',
...
// 양액탱크 목록
const nutrientTanks = [ // id, 이름, 액체 색상, svg상의 x좌표, 액체 수위 
...
const valvePositions = [21, 47, 73];   // 밸브 3개 x 위치
const nutrientTankPipeStartY = 14.4;   // 약품탱크 배관 시작 y
const nutrientManifoldY = 20.8;   // 약품이 모이는 매니폴드 y

// 좌우 위치, 구역명, 해당구역 ph/ec값, 강조 색상 
const growingZones = [
...
function PipeRun({
  d,   // SVG 경로 문자열
  tone,   // water / nutrient / steel
  width = 0.8,   // 본체 두께
  flowWidth = 0.3,   // 안쪽 흐름 선 두께
  duration = '1.8s',   // 흐름 애니메이션 속도
  reverseFlow = false,   // 유체 흐름 방향 반전 여부
}: {
  d: string;
  tone: PipeTone;
  width?: number;
